...
export class VideoCreator {
  ...
  async createVideo(jobId: number): Promise<string> {
    try {
      ...
      const job = await storage.getContentJob(jobId);
      if (!job || !job.script) {
        throw new Error(`Job ${jobId} not found or missing script`);
      }

      // Step 1: Generate TTS audio with natural voice
      console.log('Generating TTS audio...');
      const audioPath = await this.generateAudio(job.script, jobId);
      
      // Step 2: Create visual content based on video type
      console.log('Creating visual content...');
      const visualPath = await this.createVisualContent(job, audioPath);
      
      // Step 3: Apply professional editing effects
      console.log('Applying professional editing...');
      const editedPath = await this.applyProfessionalEditing(visualPath, audioPath, job);
      
      // Step 4: Final encoding and optimization
